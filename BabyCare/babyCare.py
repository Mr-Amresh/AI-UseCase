...
# 🔹 Parenting Goals to Section Mapping
GOAL_TO_SECTION = {
    "Establish a sleep routine": "sleeping_routines",
    "Introduce solid foods": "feeding",
    "Encourage developmental milestones": "milestone_tracking",
    "Improve feeding habits": "feeding",
    "Promote emotional bonding": "emotional_bonding",
    "Manage teething and health issues": "health_concerns",
    "Create a daily schedule": "daily_schedule",
    "Reduce fussiness or crying": "parenting_tips",
    "Support language development": "emotional_bonding",
    "Transition to a toddler bed": "sleeping_routines",
    "Encourage independence (e.g., self-feeding)": "parenting_tips",
    "Prepare for potty training": "parenting_tips",
    "Build a consistent nap schedule": "sleeping_routines",
    "Address allergies or sensitivities": "health_concerns",
    "General care and well-being": "parenting_tips"
}

# 🔹 Gemini Baby Guide Generation
def generate_baby_guide(baby_details, selected_goals, age_unit="months"):
    sections = set()
    for goal in selected_goals:
        sections.add(GOAL_TO_SECTION.get(goal, "parenting_tips"))  # Default to parenting_tips if no match
    
    prompt = f"""You are a baby care expert and a best baby doctor. I need your help to create a personalized baby care guide for my baby based on specific parenting goals. Please analyze the given details carefully, check the age and weight of the baby, and suggest the best advice for the selected goals only.

    I am a new parent and need personalized guidance based on the following information. Please mention if we need to visit a doctor or not:
    {baby_details}

    Provide a structured baby care guide in JSON format with only the following sections relevant to the selected goals: {', '.join(sections)}. Use this schema:
    {{
    ...
    Include only the sections specified above and leave out any others. Ensure all fields within the included sections are filled with relevant, concise information tailored for a baby whose age is in {age_unit}.
...
def generate_pdf(guide, name):
    ...
    if "sleeping_routines" in guide:
        ...
        story.append(Spacer(1, 12))
    if "milestone_tracking" in guide:
...
if age_filter == "Baby Age in Months":
    ...
    if submitted:
        if not name:
            ...
            with st.spinner("Crafting your baby’s guide... 🌟"):
                guide = generate_baby_guide(baby_profile, goals, "months")
    ...
    if submitted:
        if not name:
            ...
            with st.spinner("Crafting your baby’s guide... 🌟"):
                guide = generate_baby_guide(baby_profile, goals, "years")
